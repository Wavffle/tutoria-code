...
class ReqEvaluar(BaseModel):
    ...
    codigo_usuario: str
    salida_consola: str
    es_error_sintaxis: bool # True si Pyodide arrojó un error rojo, False si ejecutó bien
...
async def generar_ejercicio(req: ReqGenerar):
    """Genera el enunciado, la descripción y la salida esperada."""
    ...
    {{
        ...
        "salidaEsperada": "Lo que debería imprimir en consola",
...
async def pedir_pista(req: ReqPista):
    """Evalúa el código incompleto y devuelve una pista amigable."""
...
async def evaluar_codigo(req: ReqEvaluar):
    """La Capa Cognitiva: Analiza el código y la salida de Pyodide."""
...
# Agregar a la sección de Modelos de Datos
...
# Agregar a la sección de Endpoints
@app.post("/api/generar_feedback")
async def generar_feedback(req: ReqFeedback):
    """Genera la explicación detallada, la solución ideal y la decisión pedagógica post-resolución."""

    # Lógica de decisión básica (esto podría integrarse en la BD de tu plataforma)
    ...
    try:
        ...
        # Opcional: Aquí podrías inyectar datos de tu base de datos (puntos, progreso)
        # datos_ia['puntosActuales'] = ...
